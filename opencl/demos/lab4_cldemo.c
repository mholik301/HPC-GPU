...
int main(int argc, char **argv)
{
   ...
   for (size_t i = 0; i < platforms_n; i++)
   {
      ...
      devices_n = 0;
      cl_int err = clGetDeviceIDs(platforms[i], CL_DEVICE_TYPE_GPU, sizeof(devices), devices, &devices_n);
   ...
   CL_CHECK(clGetDeviceIDs(platforms[index_gpu_platform], CL_DEVICE_TYPE_GPU, sizeof(devices), devices, &devices_n));
